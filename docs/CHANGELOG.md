...
- **Design-spec implementation, P2** — contextual layout behaviors:
  - **Mode morph** — the bento ⇄ Orchestrator toggle now cross-morphs (scale + fade via `AnimatePresence`, 380ms `easeConfirm`); Orchestrator mode sits on a violet-tinted `.glass-3` field ("glass is the material of the machine"); reduced motion collapses it to a cut
  - **Glance mode** — high-distraction preset on the bento view: only the Decide tiles (hero, tasks, votes, budget) render, type steps up (`zoom: 1.06`), muted text is promoted one contrast tier, motion is disabled; hidden tiles return with one tap; persists per trip (`trip-glance:<tripId>`)
  - **Adaptive tinting** — tiles publish `--tile-dominant` (budget → cool, votes → amber, hero → brand, agent grids → AI violet); `.bento-frame` and the glass ladder pick up ~7–12% of the hue beneath
  - **Anchor-focus pulse** — `?scroll=` deep links now pulse the target tile (`accent-pulse`) after scrolling so the link teaches where content lives
