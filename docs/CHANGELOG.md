...
### Added
- **Magazine-spread POI picking** — the landing survey's picking phase is redesigned from a masonry card wall into a travel-magazine reading experience (`components/home/poi-scene.tsx`, same props/state contract): each POI is one full editorial spread — ghost folio number, kicker rule, oversized display headline, standfirst, mono fact box (likes / stay / cost), keywords line — facing a full-bleed Ken Burns photo page with a credit strip. Spreads stack vertically in a scroll-snap reader: scrolling lands on one POI at a time, with the folio counter ("03 / 24"), animated progress rule and bottom contents film-strip (picked = accent ring + order badge, trending = flame) all tracking the in-view spread via IntersectionObserver; ↑/↓/←/→ keys page, Enter/Space stamps the spread into the trip with a rotated passport-stamp "Pick Nº n", and a bobbing scroll cue marks the cover. Live trending spreads open the issue with a "Hot right now" ribbon and platform credit. Bilingual, reduced-motion-safe, guide copy updated (EN + ZH_TW)
- **POI curation & ranking analytics** — the admin console becomes a full POI management + ranking-research surface:
  - `supabase/migrations/20260707000000_poi_curation.sql` — curation columns on `poi_embeddings` (`category` — 9-value taxonomy, `labels text[]`, `curation_status` unreviewed/approved/hidden, `quality_score` 0–1 prior, `curation_notes`, `curated_at`); `search_pois_by_vibe` now excludes `hidden` POIs so curation directly gates retrieval; new aggregate RPCs `admin_poi_stats` (per-POI exposures, selections, selection rate, avg rank, picked-vs-rejected similarity split from `itinerary_feedback`) and `admin_rank_bucket_stats` (selection rate per 5-wide shortlist-rank bucket)
  - `services/admin/poi-curation.ts` (taxonomy, PATCH validation, deterministic keyword categorizer) and `services/admin/poi-analytics.ts` (stat fetchers + pure `buildCorpusOverview` rollups: by category/source/status/destination, similarity split, top/under-performers at a ≥5-exposure floor, trending-but-never-shortlisted gaps)
  - `/admin/pois` list gains category/status filters, inline curation editing (category, status, labels, quality) and live signal columns (pick rate, exposures, trend score); `PATCH /api/admin/pois/[placeId]` accepts the curation fields (metadata-only, stamps `curated_at`); new `POST /api/admin/pois/categorize` bulk-fills missing categories heuristically (no LLM, never clobbers manual labels unless `overwrite`)
  - `/admin/pois/analytics` — ranking-algorithm workbench: headline similarity-split cards, rank-bucket selection curve, category/source/status/destination rollups, top/under-performer and trending-gap lists, one-click auto-categorize
  - Hidden POIs are also excluded from the index-page trending wall (`services/home-demo/trending-pois.ts`); unit coverage in `__tests__/unit/poi-curation.test.ts`; SAD updated (EN + ZH_TW)
- **Social-media trending POIs feed the picking phase** — the itinerary generator's POI shortlist now tracks what is currently buzzing on social media instead of relying solely on the static seeded corpus:
  - `supabase/migrations/20260706000000_poi_trending_signals.sql` — `poi_trending_signals` table (RLS deny-anon): one signal per `(destination, place)` with platforms, reason, grounding evidence, buzz score (0–1) and `collected_at`; unique-upserted on re-collection
  - `services/trending/collector.ts` — three-step collection per destination: Gemini **Google Search grounding** discovers places trending on Instagram / TikTok / Reddit / YouTube / travel blogs (keyless, same approach as the `social_media_photos` agent; new `generateTextWithSearch()` in `lib/gemini.ts`), a structured `generateJson` pass extracts `{name, type, platforms, buzz, reason}` items (grounding and JSON mode are mutually exclusive), and Google Places text search resolves each to a real `place_id` — unresolvable names are dropped, never guessed. New places are upserted into `poi_embeddings` with `source='social_trending'`, geocoded via Places details (coords are backfilled on existing rows that lack them), so vibe vector search, the index-page wall and the solver can all use them
  - `services/trending/signals.ts` — read side used by the generation pipeline: `blendTrendingSignals()` runs after the MLP re-ranker, boosting in-shortlist candidates by a recency-decayed trend score (7-day half-life, 30-day expiry, max +0.15 similarity), tagging them `trending` (the LLM pick prompt now explains the tag), and injecting up to 5 hot places the vibe search missed. Fails open — no signals, identical behavior to before
  - `GET /api/cron/trending-pois` (daily, `vercel.json`) — refreshes destinations with active demand (upcoming/active trips + surveys from the last 14 days), skipping destinations refreshed within 20h, capped at 6 per run; manual warm-up via `npm run trending:collect -- --destinations "City, Country"` (`scripts/collect-trending-pois.ts`)
  - `__tests__/unit/trending-signals.test.ts` — decay/boost unit coverage; docs surface updated (SAD "Trip generator pipeline" gains a Trending signals entry, EN + ZH_TW)
  - **Index-page POI wall goes live too** — the landing survey's picking phase surfaces the same trending system instead of only the static `lib/home-survey.ts` catalog: `services/home-demo/trending-pois.ts` synthesizes `HomePoi` cards (id-namespaced `trend:<place_id>`) from `poi_trending_signals` + `poi_embeddings`, served by the new public rate-limited `GET /api/home/trending-pois?country&city` (10-min in-memory cache); the wall merges them ahead of the catalog with a flame "Trending" badge (`poi-scene.tsx`, fetch in `home-page-client.tsx`). `POST /api/home/itinerary` and `GET /api/home/poi-photo` re-resolve `trend:` ids server-side (country-checked) so client payloads stay untrusted; everything fails soft to the static catalog. Unit coverage in `__tests__/unit/home-trending-pois.test.ts`; guide + SAD updated (EN + ZH_TW)
  - **Zero-touch collection** — no manual runs needed: the daily cron now also seeds the landing-page catalog cities (always eligible, cap raised to 8/run with staleness-based rotation), and cold/stale cities self-heal on demand — `services/trending/auto-collect.ts` runs a guarded background collection via `next/server after()` when `GET /api/home/trending-pois` serves them (20h freshness window, per-instance in-flight lock + 1h attempt cooldown, catalog-bounded destination universe so the public route can't drive spend). `npm run trending:collect` remains as an optional manual warm-up. Unit coverage in `__tests__/unit/trending-auto-collect.test.ts`
