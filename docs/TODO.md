...
- [ ] **Replace boilerplate home page** — `app/page.tsx` is the default Next.js scaffold; replace with a proper landing page or redirect to the web app.
...
- [ ] **Organizer-only item actions** — Add role check (organizer vs. member) before allowing delete/reopen on web app item sheets; currently any group member can delete trip items.
...
- [ ] **Real-time web app updates** — Replace polling in the dashboard with Supabase real-time subscriptions so vote counts and board changes update live across users.
...
- [ ] **Improve web app error messages** — Replace generic "Failed to load" messages in `/api/app/*` routes with actionable, user-readable descriptions.
...
- [ ] **Bulk item operations** — Allow moving or deleting multiple items at once from the web app board view.
...
- [ ] Drag-to-reorder itinerary timeline in the web app
