  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <div className="px-4 pt-4 pb-4 space-y-4">
        ...
        <ConfirmedLinksCard links={summary.confirmedLinks} />
...
function ConfirmedLinksCard({
  links,
}: {
  links: OperationsSummary["confirmedLinks"];
}) {
  return (
    <section className="rounded-2xl border border-[var(--border)] overflow-hidden">
      <div className="px-4 py-2.5 bg-[var(--secondary)] border-b border-[var(--border)]">
        <h2 className="font-medium text-sm">Confirmed Links</h2>
      </div>
      <div className="px-4 py-3">
        {links.length > 0 ? (
          <div className="space-y-3">
            {links.map((link) => (
              <div key={link.itemId} className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-medium leading-relaxed">{link.title}</p>
          ...
          <p className="text-sm text-[var(--muted-foreground)]">
            No direct map or booking links are available for confirmed items yet.
          </p>
        )}
      </div>
    </section>
